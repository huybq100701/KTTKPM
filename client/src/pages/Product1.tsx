...
const Product1 = () => {
  ...
  const fetchComments = async () => {
    const res = await axios.get(`http://127.0.0.1:8009/comments/book/${id}`);
  ...
  useEffect(() => {
    const getApi = async () => {
      try {
        const res = await axios.get(`http://127.0.0.1:8011/books/${id}/`);
                ...
                {testData.thumbnail_images.map((item: any, index: any) => {
                  return (
                    <SwiperSlide key={index}>
                      <img src={`http://127.0.0.1:8011${item.img}`} alt="" />
                    </SwiperSlide>
                  );
                })}
              </Swiper>
